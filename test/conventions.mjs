...
test('the CI workflow shape', () => {
  ...
  // Each a bare step of check, as CONVENTIONS.md lists them: the command in a comment, in
  // another job or with `|| true` after it is a line the check job does not have.
  const check = jobLines(ci, 'check');
  for (const s of [...['lint', 'typecheck'].filter((s) => scripts[s]), 'test:conventions']) {
    assert.ok(check.includes(`      - run: npm run ${s}`), `CI runs ${s}, as a bare step of the check job`);
  }
