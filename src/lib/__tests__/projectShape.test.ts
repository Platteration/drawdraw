...
const project = (over: Record<string, unknown> = {}) => ({ id: 'p1', updatedAt: 5, image: IMAGE, ...over });

/** The settings a record comes back as, when it comes back at all: a refusal fails the test here. */
function settingsFrom(raw: unknown): ProjectSettings {
  const out = sanitizeSettings(raw);
  if (!out) throw new Error('sanitizeSettings refused the record');
  return out;
}

/** The project a record comes back as, when it comes back at all: a refusal fails the test here. */
function projectFrom(raw: unknown): Project {
  const out = sanitizeProject(raw);
  if (!out) throw new Error('sanitizeProject refused the record');
  return out;
}
 ...
 */
const pathData = (transform: HeadTransform, proportions?: Partial<Proportions>) => {
...
describe('sanitizeProject', () => {
  ...
  it('opens nothing but a local file', () => {
    ...
    expect(projectFrom(project()).image.uri).toBe(IMAGE.uri);
  });
  ...
  it('refuses a file URI that climbs out of the directory it names', () => {
    ...
    for (const uri of [`${dir}./p1.jpg`, `${dir}p1..jpg`, `${dir}..p1.jpg`]) {
      expect(projectFrom(project({ image: { ...IMAGE, uri } })).image.uri).toBe(uri);
    }
  ...
  it('gives an unreadable timestamp a value that still sorts', () => {
    ...
    expect(projectFrom(project({ updatedAt: undefined })).updatedAt).toBe(0);
    expect(projectFrom(project({ updatedAt: 'yesterday' })).updatedAt).toBe(0);
  });
});
...
describe('sanitizeSettings', () => {
  ...
  it('keeps a complete, legitimate settings blob', () => {
    const third = GUIDE_COLORS[2];
    if (!third) throw new Error('the palette has fewer than three guide colours');
    const saved = {
      showHead: false,
  ...
  it('drops a half-written head pose rather than emitting NaN geometry', () => {
    ...
    for (const key of Object.keys(full)) {
      ...
      delete partial[key];
      expect(settingsFrom({ headTransform: partial }).headTransform).toBeUndefined();
    }
    expect(settingsFrom({ headTransform: { ...full, scale: 0 } }).headTransform).toBeUndefined();
    expect(settingsFrom({ headTransform: { ...full, yaw: NaN } }).headTransform).toBeUndefined();
    expect(settingsFrom({ headTransform: full }).headTransform).toEqual(full);
  });
  ...
  it('drops a pose whose numbers are finite but not a pose', () => {
    ...
    for (const over of [
      ...
      expect(settingsFrom({ headTransform: { ...POSE, ...over } }).headTransform).toBeUndefined();
    }
  });
  ...
  it('keeps the poses the app itself produces', () => {
    ...
    const taps = (chin: number, nose: number, brow: number) => [
    ...
    for (const spread of [taps(430, 330, 280), taps(495, 120, 20), taps(300, 290, 285)]) {
      ...
      expect(settingsFrom({ headTransform: solved }).headTransform).toEqual(solved);
    }
  });
  ...
  it('keeps the pose a fit on a photo nothing is shaped like produces', () => {
    ...
    const cases: { taps: [number, number][]; clampedTo: number }[] = [
  ...
  it('keeps every proportion preset the app offers', () => {
    for (const preset of PROPORTION_PRESETS) {
      expect(settingsFrom({ proportions: preset.values }).proportions).toEqual({
        ...DEFAULT_PROPORTIONS,
        ...preset.values,
      });
    }
  ...
  it('completes proportions from the defaults, the way the model does', () => {
    ...
    const out = settingsFrom({ proportions: { browY: 0.28 } }).proportions;
    expect(out).toEqual({ ...DEFAULT_PROPORTIONS, browY: 0.28 });
    const byName: Record<string, unknown> = { ...out };
    for (const key of Object.keys(DEFAULT_PROPORTIONS)) {
      expect(typeof byName[key]).toBe('number');
      expect(Number.isFinite(byName[key])).toBe(true);
    }
    expect(settingsFrom({ proportions: { browY: 'low' } }).proportions).toBeUndefined();
    expect(settingsFrom({ proportions: 4 }).proportions).toBeUndefined();
    ...
    expect(settingsFrom({ proportions: { width: 0 } }).proportions).toBeUndefined();
    expect(settingsFrom({ proportions: { width: 2, browY: 0.3 } }).proportions).toEqual({
      ...DEFAULT_PROPORTIONS,
      browY: 0.3,
    });
  });
  ...
  it('drops a proportion with no range rather than throwing on it', () => {
    ...
    jest.isolateModules(() => {
      jest.doMock('../headModel', () => {
        const actual = jest.requireActual<typeof import('../headModel')>('../headModel');
        return { ...actual, DEFAULT_PROPORTIONS: { ...actual.DEFAULT_PROPORTIONS, jawWidth: 1 } };
      });
      // A fresh copy, inside this registry, that reads the headModel above.
      const shape = jest.requireActual<typeof import('../projectShape')>('../projectShape');
  ...
  it('holds the two appearance sliders to their own ranges', () => {
    ...
    const widthOf = (thickness: number, bucket: number) => thickness * (0.7 + (0.55 * bucket) / 3);
    expect(widthOf(1.7e308, 2)).toBe(Infinity);
    expect(settingsFrom({ lineWeight: 1.7e308 }).lineWeight).toBeUndefined();

    for (const v of [0, -1, 0.5, 6.5, 100]) {
      expect(settingsFrom({ lineWeight: v }).lineWeight).toBeUndefined();
    }
    for (const v of [...LINE_WEIGHT_RANGE, 2, 3.5]) {
      expect(settingsFrom({ lineWeight: v }).lineWeight).toBe(v);
    }
    for (const v of [0, 0.01, 0.9, 1, 1e308]) {
      expect(settingsFrom({ tracingOpacity: v }).tracingOpacity).toBeUndefined();
    }
    for (const v of [...TRACING_OPACITY_RANGE, 0.3]) {
      expect(settingsFrom({ tracingOpacity: v }).tracingOpacity).toBe(v);
    }
  });
  ...
  it('never adds an element back in', () => {
    ...
    const out = settingsFrom({ elements: { segments: true } }).elements;
    expect(out).toEqual({ segments: true });
    for (const key of proKeys) expect(out?.[key]).toBeUndefined(); // out is { segments: true }, above

    // An all-off set is a legitimate thing to have saved.
    expect(settingsFrom({ elements: {} }).elements).toEqual({});
    // ...and only a real `true` counts.
    expect(settingsFrom({ elements: { jaw: 'yes', ears: 1, mouthLine: true } }).elements).toEqual(
      { mouthLine: true }
    );
    expect(settingsFrom({ elements: { notAnElement: true } }).elements).toEqual({});
    expect(settingsFrom({ elements: null }).elements).toBeUndefined();
  });
  ...
  it('drops values outside their own domain instead of inventing one', () => {
    ...
    expect(settingsFrom({ hGuides: [0.3, 1.5] }).hGuides).toBeUndefined();
    expect(settingsFrom({ hGuides: [-0.2] }).hGuides).toBeUndefined();
    expect(settingsFrom({ vGuides: new Array(1000).fill(0.5) }).vGuides).toBeUndefined();
    expect(settingsFrom({ hGuides: [1 / 3, 2 / 3] }).hGuides).toEqual([1 / 3, 2 / 3]);
  ...
  it('only takes a guide colour the app actually offers', () => {
    for (const c of GUIDE_COLORS) {
      expect(settingsFrom({ guideColor: c.value }).guideColor).toBe(c.value);
    }
    expect(settingsFrom({ guideColor: '#ff0000' }).guideColor).toBeUndefined();
  });
});
